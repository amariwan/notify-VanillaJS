const notify = (option) => {
	...
	const _Create = () => {
		...
		_Element.classList.add('tasio–notify', type.toLowerCase().trim());
		size === true ? _Element.classList.add(`tasio–notify-${size}`) : null;
		console.log(`tasio–notify-${size}`);
		overlay === true ? _Element.classList.add('tasio–notify-overlay') : null;
		overflowHide === true ? _Element.classList.add('tasio–notify-overflow-hide') : null;
		spacing != null ? (_Element.style.margin = `${spacing}px`) : null;
		theme === 'dark' ? _Element.classList.add('tasio–notify-dark') : null;

		if (position != null) {
			_position = position;
			if (position.x != null) {
				_Element.classList.add(`tasio–notify-${position.x}`);
			}
			if (position.y != null) {
				_Element.classList.add(`tasio–notify-${position.y}`);
			}
		} else {
			_Element.style.right = '10px';
			_Element.style.bottom = '10px';
			_Element.classList.add('tasio–notify-right-bottom');
		}

		if (closeBtn === true) {
			const _CloseButton = document.createElement('div');
			_CloseButton.classList.add('tasio–notify-close-btn');
	...
	const _CheckPosition = () => {
		const notifElements = document.querySelectorAll('.tasio–notify');
